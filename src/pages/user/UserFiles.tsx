...
const UserFiles = () => {
  ...
  const handleUploadFolder = () => {
    ...
    input.onchange = async (e) => {
      ...
      if (uploadedFiles && uploadedFiles.length > 0) {
        ...
        const totalSize = Array.from(uploadedFiles).reduce(
          (sum, f) => sum + f.size,
          0
        );
      ...
      try {
        ...
        toast({
          ...
          description:
            ...
            `Failed to create folder "${folderName}". Please try again.`,
          variant: "destructive",
        });
      }
    }
  ...
  const handleDelete = async (item: FileItem) => {
    try {
      await deleteFileOrFolder.mutateAsync({
        itemId: item.id,
        itemType: item.type,
      });
      toast({
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col space-y-3 lg:flex-row lg:items-center lg:justify-between lg:space-y-0">
        <div className="min-w-0">
          ...
          <BreadcrumbNav items={currentPath} onNavigate={handleNavigate} />
        </div>
        <FileToolbar
