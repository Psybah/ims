...
export const deleteFileOrFolder = (
  ...
  const endpoint =
    itemType === "file" ? `/files/file/${itemId}` : `/files/folder/${itemId}`;
...
export const fetchFolderById = async (folderId: string) => {
  const { data } = await apiV2.get(
    `/files/folders/${folderId}?resourceType=FOLDER`
  );
  return data.data;
};
...
export const deleteFile = async (fileId: string) => {
  const { data } = await apiV2.delete(
    `/files/file/${fileId}?resourceType=FILE`
  );
  return data;
};
...
export const useCreateFolderMutation = () => {
  ...
  return useMutation<any, unknown, CreateFolderArgs>({
    mutationFn: ({ folderName, parentId }) =>
...
export const useUploadFileMutation = () => {
  ...
  return useMutation<any, unknown, UploadFileArgs>({
    mutationFn: ({ file, parentId, onUploadProgress }) =>
      uploadFile(file, parentId, onUploadProgress),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["folders"] });
    },
  });
...
export const useUploadFolderMutation = () => {
  ...
  >({
    mutationFn: ({ files, selectedFolder, parentId, onUploadProgress }) =>
      uploadFolder(files, selectedFolder, parentId, onUploadProgress),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["folders"] });
    },
  });
