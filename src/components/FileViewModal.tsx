...
function reconstructGoogleViewLink(url: string) {
  ...
  if (mimeType.startsWith("image/") || fileType.includes("image")) {
    return (
      <div className="flex items-center justify-center bg-muted/30 rounded-lg p-4">
        {file.webViewLink ? (
          <iframe
            src={reconstructGoogleViewLink(file.webViewLink)}
...
export function FileViewModal({
  ...
  const handleRename = () => {
    ...
    toast({
      ...
      description:
        "File rename functionality is not yet supported by the backend.",
      variant: "destructive",
    });
